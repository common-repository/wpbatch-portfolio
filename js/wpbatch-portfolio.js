jQuery(window).load(function(){
		...
		// the main container
	var $GPContainer	= jQuery('#container'),
		// the articles (the thumbs)
		$articles		= $GPContainer.children('div.block'),
		// total number of articles
		totalArticles	= $articles.length,
		// the fullview container
		$fullview		= jQuery('<div id="fullview" class="full-view-elements"></div>').prependTo( jQuery('body') ),
		// the overlay
		$overlay		= jQuery('<div class="overlay"></div>').prependTo( jQuery('body') ),
			...
			var animspeed				= 500,
				...
				initEventsHandler		= function() {
					...
					$articles.each( function(i) {
						jQuery(this).find('a.more_link').bind('click.GridPortfolio', function(e) {
							
							if( animrun ) return false;
							animrun			= true;
							
							var $article	= jQuery(this).closest('div.block');
					...
					// fullview navigation
					jQuery('#fullview').find('span.full-nav-next').live('click.GridPortfolio', function(e) {
						if( animrun ) return false;
						animrun	= true;
					...
					jQuery('#fullview').find('span.full-nav-prev').live('click.GridPortfolio', function(e) {
						if( animrun ) return false;
						animrun	= true;
					...
					jQuery('#fullview').find('span.full-view-exit').live('click.GridPortfolio', function(e) {
					...
					jQuery(window).bind('resize.GridPortfolio', function(e) {
				...
				showArticle				= function( $article, anim ) {
					...
					var clonestyle 	= {
						width	: jQuery(window).width() - padding + 'px',
						height	: jQuery(window).height() - padding + 'px',
						left	: '0px',
						top		: jQuery(window).scrollTop() + 'px'
					};
					
					$clone.appendTo( jQuery('body') ).stop().applyStyle( clonestyle, $.extend( true, [], { duration : animspeed, easing : animeasing, complete : function() {
						...
						var $fullview_content	= jQuery('#fullviewTmpl').tmpl( articleFullviewData );
					...
					var thumbsstyle 	= {
						left	: jQuery(window).width() - $thumbsWrapper.width() - 25 + 'px',  // 25 is the margin left / right of the fullview thumbs-wrapper
						top		: (jQuery(window).height() / 2) - ($thumbsWrapper.height() / 2) - 22 + 'px' // 10 is the margin top / bottom of the fullview thumbs-wrapper
					};
				...
				showFullviewArticle		= function() {
					var $article					= $articles.eq( current ),
						...
						$newFullviewThumbsWrapper	= jQuery('<div class="thumbs-wrapper"><div class="thumbs">' + articleFullviewData.thumbs + '</div></div>');
					
					// preload the article's background image
					$loading.show();
					jQuery( articleFullviewData.bgimage ).load(function() {
						$loading.hide();
						var $bgImage	= jQuery(this);
